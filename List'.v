...
Inductive _list (p : Obj) (A : forall p0 : Obj, p ≤ p0 -> forall p : Obj, p0 ≤ p -> Type) : Type :=
| _nil : _list p A
| _cons : (forall (p0 : Obj) (α : p ≤ p0), A p0 α p0 #) ->
          (forall (p0 : Obj) (α : p ≤ p0), _list p0 (fun (q : Obj) (α0 : p0 ≤ q) => A q (α ∘ α0))) ->
          _list p A
.

Forcing Definition list : Type -> Type using Obj Hom.
Proof.
intros p A q f; exact (_list q (fun r g => A r (f ∘ g))).
Defined.
...
Definition bar := fun A (P : Type) (H0 : P) (HS : A -> list A -> P -> P) (x : A) (l : list A) => list_rec A P H0 HS (cons _ x l).
...
Definition list_mem : forall A R, list A -> (list A -> R) -> R :=
  fun A R : Type =>
    list_rec A ((list A -> R) -> R) (fun f => f (nil _))
             (fun a _ H f => H (fun ll => f (cons _ a ll))).
...
(fix
 list_rec_ (p0 : Obj) (A0 P0 : forall p1 : Obj, p0 ≤ p1 -> forall p2 : Obj, p1 ≤ p2 -> Type)
           ...
           (l : _list p0 (fun (r : Obj) (g : p0 ≤ r) => A0 r (fun (R : Obj) (k : Hom r R) => g R k))) {struct l} :
   P0 p0 # p0 # :=
   match l with
   | _nil _ _ => Hnil0 p0 #
   | _cons _ _ ny ll =>
  ...
  (fun (p0 : Obj) (α : p ≤ p0)
   ...
   f p0 #
     (fun (p1 : Obj) (α0 : p0 ≤ p1) =>
      _nil p1 (fun (r : Obj) (g : p1 ≤ r) => A r (fun (R : Obj) (k : Hom r R) => α R (α0 R (g R k))))))
  (fun (p0 : Obj) (α : p ≤ p0) (a : forall (p1 : Obj) (α0 : p0 ≤ p1), A p1 (α ∘ α0) p1 #)
     (_ : forall (p1 : Obj) (α0 : p0 ≤ p1),
          _list p1 (fun (r : Obj) (g : p1 ≤ r) => A r (fun (R : Obj) (k : Hom r R) => α R (α0 R (g R k)))))
